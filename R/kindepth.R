...
#' `min(depth)` if `depth` reach a state with no founders
#' is not possible.
...
#'     c("E", "E", "0", "0", "0")
#' )
setMethod("kindepth", "character_OR_integer",
    function(
        obj, dadid, momid,
        ...
        for (i in seq_len(n)) {
            ...
            ## Keeping it here because it was not the issue being fixed
            ## in 9/2023.
        ...
        ## the general algorithm below: the result would depend on the data
        ## order.
        ## 2. Find any remaining mother-father pairs that are mismatched in
        ## depth. Deal with them one at a time.
        ## 3. The children's depth is max(father, mother) +1.
        ## Call the parent closest to the children ``good'' and the other
        ## ``bad''.
        ...
        ## We do not followany connections at a depth lower than the marriage
        ## in question, to getthe highest marriages right.
        ## For the bad side, just get ancestors.
        ## 5. Avoid pedigree loops!  If the agood list contains anyone in abad,
        ## then don't try to fix the alignment, otherwise: Push abad down,
        ## then run the pushdown algorithm to repair any descendents.
        ## You may have pulled down a grandparent but not the sibs of that
        ## grandparent.

        ## It may be possible to do better alignment when the Pedigree has
        ## loops, but it is definitely beyond this program, perhaps in
        ## auto_hint one day.

        ## First deal with any parents who are founders with depth 0
        dads <- didx[midx > 0 & didx > 0]  # the father side of all pairs
        moms <- midx[midx > 0 & didx > 0]
        if (0) {
            ## somewhere between version 1.8.5 and 1.9.6 this portion added
            ## check against other test cases, but was trying to fix where a
            ## mother had multiple marriages
        ...
        while (TRUE) {
            ...
            if (length(abad) == 1 && sum(c(dads, moms) == bad) == 1) {
                ...
                while (TRUE) {
                    ...
                    ## Add kids
                    kids <- (
                        !is.na(match(midx, temp)) |
                            !is.na(match(didx, temp))
                    )
                ...
                if (all(match(abad, agood, nomatch = 0) == 0)) {
                    ...
                    ##  below their parents.
                    ## It's easier to run through the whole tree
                    for (i in 0:n) {
            ...
            ## Once a subject has been shifted, we don't allow them to
            ## instigate yet another shift, possibly on another level
            done[who] <- TRUE
