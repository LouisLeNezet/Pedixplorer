...
ped_server <- shiny::shinyServer(function(input, output, session) {
    ...
    cols_needed_ped2 <- data_col_sel_server(
        "data_ped_col_sel2", ped_df,
    ...
    ped_df_aff <- shiny::reactive({
        ...
        if (!is.null(ped_df_fam())) {
            tryCatch({
                ped_df_aff <- print_console(
                    generate_aff_inds(ped_df_fam(),
