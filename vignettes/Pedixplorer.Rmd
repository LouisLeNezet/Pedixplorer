...
The text size and distance from the box subject for each label can be changed with the
`label_cex` and `label_dist` arguments of the `plot()` method.
...
```{r, label_size, fig.alt = "Pedigree of family 1 with change in label size and distance"}
