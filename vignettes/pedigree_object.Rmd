...
- `ped_df` A dataframe containing the columns
    ...
    - `avail` Optional, a numeric variable with `0`=unavailable
    and `1`=available.
    - `affected` Optional, a numeric variable with `0`=unaffected
    and `1`=affected.
    - `deceased` Optional, a numeric variable with `0`=alive and
    `1`=dead.
...
- `steril`, `deceased`, `avail`, `affected` only contains 0, 1 or NA values
...
- Other infos used : `affected()`, `avail()`, `deceased()`
...
```{r, pedigree_methods, fig.alt = "Pedigree plot"}
